/* Atheer Test Lab — browser-only mock simulator (no server) */
...
function hashId(input){
  // tiny deterministic hash for scenario id
  let h = 2166136261;
...
function saveRuns(){
  localStorage.setItem("atheer_runs_v1", JSON.stringify(state.runs.slice(-20)));
}

function loadRuns(){
  try{
    const raw = localStorage.getItem("atheer_runs_v1");
    if (!raw) return [];
...
function simulateRun(scenario){
  ...
  for (let i=0;i<scenario.n;i++){
    ...
    if (drop){
      retries++;
      // second attempt: some succeed, some fail
      const secondFail = rand() < 0.12;
      t += scenario.latency + Math.abs((rand()*2-1)*scenario.jitter) + 40;
      err = secondFail;
    } else {
      // occasional host/risk fail
      err = rand() < 0.008;
    }

    // risk engine adds latency when stressed
